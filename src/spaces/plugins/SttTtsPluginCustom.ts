// src/plugins/SttTtsPluginCustom.ts
...
export class SttTtsPlugin implements Plugin {
  ...
  private async askChatGPT(userText: string): Promise<string> {
    try {
      const agentUrl = `http://localhost:3000/agent/agentA/ask`;
      const body = { userInput: userText };
      ...
      if (!resp.ok) {
        ...
        console.error('[SttTtsPlugin] Agent call error =>', errText);
        return 'Agent is unavailable at the moment.';
      }

      const data = await resp.json();
      const reply = data.botResponse || 'No response from agent.';

      // Optionally store the Q&A in chatContext
      this.chatContext.push({ role: 'user', content: userText });
      this.chatContext.push({ role: 'assistant', content: reply });

      return reply;
    } catch (error) {
      console.error('[SttTtsPlugin] Error =>', error);
